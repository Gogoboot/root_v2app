...
use super::constants::{
    DROPS_PER_SAP, PRICE_DROP_TRIGGER_PCT, STABFUND_RESERVE_PCT,
    TREASURY_LOW_THRESHOLD_PCT, TREASURY_MIN_RESERVE_PCT,
};
...
impl Treasury {
    ...
    pub fn deposit(&mut self, amount: u64, from_slash: bool) {
        ...
        if from_slash { self.total_slash_drops += amount; }
        else          { self.total_fees_drops  += amount; }
    }
    ...
    pub fn reward_multiplier(&self, total_supply: u64) -> f64 {
        ...
        if self.balance_drops < min      { 0.0 }
        else if self.balance_drops < low { 0.5 }
        else                             { 1.0 }
    }
    ...
    pub fn check_stabfund_intervention(&mut self, current_price: f64) -> Option<u64> {
        ...
        if now - self.last_price_ts < 3600 { return None; }
